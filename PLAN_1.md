...
- Windows 后端已按 64 位 CLSID 注册绝对路径动态加载原厂 DLL，仅解析经过文档确认的
  ...
  read-modify-write 开放固件 `0x018A` 的 Input 1/2 前级增益、48V、80 Hz 高通、相位反转
  、三组物理输出增益与硬件输出静音。
  raw vendor、固件和 DFU 导出仍不解析。
- 已增加显式 ignored 的 Windows 真机只读与写入恢复测试；本机连接的 Stream 4x5 已
  通过枚举、打开、snapshot 读取和关闭验证；Input 1/2 与三组输出增益分别临时变化
  1 dB，48V、高通与相位状态分别临时反转后均回读匹配，并成功恢复原值且再次回读
  确认；六个输出声道的原始 Q8.8 值均得到精确恢复。Linux 后端仍为安全骨架。
...
- 已用 test-only `TUSBAUDIO_SetDspProperty` 绑定验证 Windows DSP output mute 的
  property 400 SET：选取当前非静音输出对，保存两个原始 Q8.24 值，写零并回读后逐值
  恢复且再次回读一致。由于启动时已静音的输出对无法从 property 400 单独恢复其路由、
  master attenuation、solo 与 pad 共同决定的解静音值，生产后端不使用此 DSP 路径。
- 已进一步静态确认 40 字节块 `12..17` 是六路硬件输出增益到达 `-60 dB` 时由原厂
  serializer 写入的成对标志，不是原厂可见 DSP mute。gated ignored 真机测试已在
  Control Center 关闭时选择 Output 1/2，通过公开 encoder 只改对应一对标志并持续 5 秒；
  两字节及公开 Boolean 状态均回读为 `1`，用户确认物理输出在观察窗内静音，随后完整
  原始 40 字节块逐字节恢复一致。Windows 固件 `0x018A` 已开放三组硬件输出静音；GUI
  状态 pill 在该 capability 下成为反向切换按钮，并由 controller 执行写入和强制回读。
  输出增益写入也与原厂 serializer 一致：`-60 dB` 置位对应标志，高于端点时清位。
- GUI 已接入真实 platform backend 与 controller worker，提供设备连接状态、手动/周期
  刷新、设备/固件/采样率/时钟/ASIO buffer 概览；固件 `0x018A` 上 Input 1/2 前级增益
  与三组物理输出增益以 1 dB 滑杆写入并由 worker 约 30 Hz 合并，48V、80 Hz 高通与
  相位及三组硬件输出静音以按钮立即写入；Input 1/2 推子端点状态仍只读。
  Input 3/4 明确显示为固定硬件
...
- 已有静态 ABI 分析、脱敏读/写 fixture、一台 Stream 4x5 的 Windows vendor-API
  读验证、Rust backend 真机 snapshot 验证及 Input 1/2 增益、48V、高通、相位与三组
  输出增益写入/回读/恢复验证；尚无
  USBPcap 原始总线证据、Linux `rusb` 直连或 Windows DSP meter 动态验证。除固件
  `0x018A` 的 Windows Input 1/2 增益、48V、高通、相位、三组输出增益及硬件输出静音外，其他真实写
  控制、实时电平和 Lewitt XML 映射仍不得标记为可用。
...
- Windows Input 1/2 增益、48V、高通、相位、三组输出增益、公开硬件输出静音与
  test-only DSP output mute 的显式 ignored 硬件写入恢复测试已通过。sample-rate gated 测试第一次因
  `TSTATUS_ASIO_IN_USE` 被安全拒绝；关闭客户端后，设备只报告 `[48000]`，写入
  `44100` 返回 `TSTATUS_INVALID_SAMPLE_RATE`，写入当前值 `48000` 并回读通过。
  因没有第二个设备报告支持的值，尚未完成真实切换/恢复，公共写入继续禁用。以后新增
  真机写测试仍必须保持显式 opt-in，并在断言前恢复测试前状态。

### 下一实施批次

1. 继续只读恢复 Windows streaming mode/stream format 状态，解释设备为何仅报告
   `48000`；不得为凑测试结果切换 streaming mode。只有设备先报告第二个支持值，才重新
   运行 gated sample-rate 临时切换、回读和恢复测试并考虑开放能力。
2. 在 Arch 环境编译并运行验证 `XDG_RUNTIME_DIR` Unix socket、KSNI 托盘和退出清理，
   并使用系统 `libusb` 实现仅 VID/PID 的只读发现与权限诊断，验证不解绑
   `snd-usb-audio` 时能否发送 interface-recipient EP0 只读请求。
3. 用 USBPcap 确认文档中的 setup packet，一次只抓取、验证并开放一个 Linux 属性。
4. 为已确认的硬件输出静音补充 USBPcap 与重新连接持久性验证；在此之前 Linux 仍不
   声明 `OutputMute` capability。
5. 为 Linux 物理输入/输出 RMS 另立被动 PipeWire/ALSA meter provider，不混入 USB
   后端，不实现混音、虚拟通道、Ducker 或 FX。
6. 最后实现 Windows 当前用户启动项与 Arch XDG autostart 写入层，并保持显式确认门禁。
...
增益、48V、80 Hz 高通、相位与三组输出增益写入/回读/恢复；其他真实写入和未经验证
的 DSP meter 仍禁止。Linux 只读发现及全部 USB 协议操作尚未实现。
...
Windows 命名管道和首版硬件面板已完成，Input 1/2 与三组输出增益滑杆、48V、80 Hz
高通与相位按钮已接入写入 worker，并通过 Windows 编译及测试；Linux Unix
